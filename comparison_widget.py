        ...
        try:
            ...
            for name, path in self.model_entries:
                ...
                metrics, matches, fp, fn = evaluate_model(
                    ...
                    manual_attrs=self.manual_attrs,
                )
                ...
                if metrics.get("class_aware"):
                    self.log.emit("  -> Matching memperhitungkan KELAS (GT & hasil deteksi sama-sama punya atribut kelas).")
                else:
                    self.log.emit("  -> Matching hanya berdasarkan posisi (info kelas tidak lengkap di GT/hasil deteksi).")
                results.append({
